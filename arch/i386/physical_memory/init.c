 ...
 * 2. map physical aera after kernel to store bitset for the physical memory
 * manager (pmm)
...
static void
__create_physical_region(uint32_t len, uint32_t addr, uint32_t type) {
	...
	if (bytes > available_bytes) {
		uintptr_t paddr =
				_physical_mmap_end + (0x1000 - _physical_mmap_end % 0x1000);
	...
	pmm_region_t* new_region =
			(pmm_region_t*)PHYSICAL_PTR_TO_VIRTUAL((uint32_t*)_physical_mmap_end);
...
static void
__read_physical_memory_regions_block(multiboot_mmap_region_t* mmap) {
	...
	if (mmap->addr < 0x100000 && mmap->type == MULTIBOOT_MEMORY_AVAILABLE) {
		...
		__create_physical_region(0x1000000 - mmap->addr, mmap->addr,
		                         MEMORY_LOWER_REGION);
...
static __inline void __display_usable_physical_memory() {
	...
	for (pmm_region_t* idx = physical_memory_map; idx; idx = idx->next) {
		printk("   [%#x : %#x] type: %d, %d Kib of usable memory\n", idx->pstart,
		       idx->pend, idx->type, idx->page_nb * 4);
...
static __inline void __map_used_physical_region() {
	uint32_t kernel_start_addr =
			VIRTUAL_ADDR_TO_PHYSICAL((uint32_t)&_kernel_start);
	uint32_t kernel_end_addr = VIRTUAL_ADDR_TO_PHYSICAL((uint32_t)&_end);
	kernel_end_addr -= kernel_end_addr % 0x1000;
	uint32_t result =
			pmm_set_region(kernel_start_addr, kernel_end_addr, PM_PAGE_PRESENT);
	if (result != EXIT_SUCCESS) {
		PANIC("Could not map physical aera of the kernel");
	}
	result = pmm_set_region(_physical_mmap_start, _physical_mmap_end - 0x1000,
	                        PM_PAGE_PRESENT);
	if (result != EXIT_SUCCESS) {
...
void physical_memory_init(multiboot_info* header) {
	...
	if (pg_map(_physical_mmap_start,
	           PHYSICAL_ADDR_TO_VIRTUAL(_physical_mmap_start), 0x3,
	           false) == EXIT_FAILURE) {
		PANIC("Fatal error, could not map physical region, more RAM needed");
	}
	mltb_foreach_physical_memory_region(header,
	                                    &__read_physical_memory_regions_block);
