...
void pmm_release_page(uintptr_t addr) { pmm_set_page((uint32_t)addr, false); }

uintptr_t __compute_addr_and_alloc(pmm_region_t* region,
                                   uint32_t      offset,
                                   uint32_t      bit,
                                   uint32_t      bit_count) {
	uintptr_t addr =
			region->pstart + (offset * 32 * 0x1000) + (bit_count * 0x1000);
	region->bitset[offset] |= bit;
	return addr;
}

uintptr_t pmm_get_page(uint8_t type) {
	for (pmm_region_t* region = physical_memory_map; region;
	     region               = region->next) {
		unsigned idx = 0;
