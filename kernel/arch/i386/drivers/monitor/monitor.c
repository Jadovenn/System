...
static int get_offset_from_chartesian(int x, int y) { return (2 * (y * COLONE_MAX + x)); }
...
static void scroll_down()
{
	...
	for (int count = 0; count < dest_offset; count += COLONE_MAX << 1) {
		memcpy(VIDEO_MEMORY_BUFFER_PTR + count, VIDEO_MEMORY_BUFFER_PTR + count + (COLONE_MAX << 1), COLONE_MAX << 1);
	}
...
void monitor_set_color(unsigned char fg, unsigned char bg) { cursor_color = (char)(fg | bg << 4); }
