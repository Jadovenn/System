/**
 * i386_entry.c - x86 kernel entry point
...
void check_multiboot(Multiboot_info_t* header, uint32_t magic)
{
	...
	if (!MULTIBOOT_TEST_FLAG(header, MULTIBOOT_INFO_MEMORY)) {
		PANIC("Multiboot memory flags is not present, manual memory detection "
		      "is not available");
	}
	...
	if (!(header->flags & MULTIBOOT_INFO_DRIVES)) {
		PANIC("Multiboot devices map is not present.");
	}
