...
static void _create_physical_region(uint32_t len, uint32_t addr, uint32_t type)
{
	...
	if (bytes > available_bytes) {
		uintptr_t paddr = G_Physical_mmap_end + (0x1000 - G_Physical_mmap_end % 0x1000);
	...
	Physical_memory_region_t* new_region =
			(Physical_memory_region_t*)PHYSICAL_PTR_TO_VIRTUAL((uint32_t*)G_Physical_mmap_end);
...
static void _read_physical_memory_regions_block(Multiboot_mmap_region_t* mmap)
{
	...
	} else if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE && mmap->addr + mmap->len - 1 < 0x1000000) {
		_create_physical_region(mmap->len, mmap->addr, mt_AVAILABLE);
	} else if (mmap->type == MULTIBOOT_MEMORY_AVAILABLE && mmap->addr < 0x1000000 &&
	           mmap->addr + mmap->len - 1 > 0x1000000) {
		_create_physical_region(0x1000000 - mmap->addr, mmap->addr, mt_LOWER_REGION);
		_create_physical_region(mmap->len - (0x1000000 - mmap->addr), mmap->addr + (0x1000000 - mmap->addr), mmap->type);
	} else {
...
static __inline void _display_usable_physical_memory()
{
	...
	for (Physical_memory_region_t* idx = G_Physical_memory_map; idx; idx = idx->next) {
		printf("   [%#x : %#x] type: %d, %d Kib of usable memory\n", idx->startAddr, idx->endAddr, idx->type,
		       idx->page_nb * 4);
...
{
	uint32_t kernel_start_addr = VIRTUAL_ADDR_TO_PHYSICAL((uint32_t)&G_Start_kernel);
	uint32_t kernel_end_addr   = VIRTUAL_ADDR_TO_PHYSICAL((uint32_t)&G_End_kernel);
	kernel_end_addr -= kernel_end_addr % 0x1000;
	uint32_t result = Physical_memory_set_region(kernel_start_addr, kernel_end_addr, pms_PRESENT);
	if (result != EXIT_SUCCESS) {
		PANIC("Could not map physical area of the kernel");
	}
	result = Physical_memory_set_region(G_Physical_mmap_start, G_Physical_mmap_end - 0x1000, pms_PRESENT);
	if (result != EXIT_SUCCESS) {
...
void Init_physical_memory(uintptr_t aMultibootMmapAddr, uint32_t aMultibootMmapLength)
{
	...
	if (Boot_paging_map(G_Physical_mmap_start, PHYSICAL_ADDR_TO_VIRTUAL(G_Physical_mmap_start), 0x3, false) ==
	    EXIT_FAILURE) {
		PANIC("Fatal error, could not map physical region, more RAM needed");
	}
	Multiboot_mmap_region_t* mmapRegion = NULL;
	for (mmapRegion = (Multiboot_mmap_region_t*)PHYSICAL_PTR_TO_VIRTUAL(aMultibootMmapAddr);
	     (uint32_t)mmapRegion < PHYSICAL_ADDR_TO_VIRTUAL(aMultibootMmapAddr) + aMultibootMmapLength;
	     mmapRegion = (Multiboot_mmap_region_t*)((uint32_t)mmapRegion + mmapRegion->size + sizeof(mmapRegion->size))) {
