...
void _heapBM_init(_heapBM_t* heap) { heap->fblock = NULL; }

void _heapBM_add_block(_heapBM_t* heap, uintptr_t addr, uint32_t size,
                       uint32_t block_size) {
...
static __inline__ uint8_t _heapBM_get_node_id(uint8_t a, uint8_t b) {
	uint8_t c;
	for (c = a + 1; c == b || c == 0; ++c)
		;
	return c;
...
void* _heapBM_alloc(_heapBM_t* heap, size_t size) {
	...
	for (block = heap->fblock; block; block = block->next) {
		if (block->size - (block->used * block->bsize) >=
		    size) { // Regular case: block as enought slot ?
...
void _heapBM_free(_heapBM_t* heap, void* ptr) {
	...
	for (block = heap->fblock; block; block = block->next) {
		if ((uintptr_t)ptr > (uintptr_t)block &&
		    (uintptr_t)ptr <
		        (uintptr_t)block + sizeof(_heapBM_block_t) + block->size) {
